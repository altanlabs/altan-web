...
export function IntegrationCard({ item, index }: CardProps) {
  const appLink = item.name.toLowerCase().trim().replace(/\s+/g, '-');
  return (
      ...
      <div className="flex flex-col p-5 h-full">
        <div className="flex items-center space-x-3 mb-3">
          <div className="relative flex-shrink-0">
            <IconRenderer icon={item.icon} size={40} />
          </div>
          <Link className="font-semibold bg-clip-text text-transparent bg-gradient-to-r from-slate-200/60 via-slate-200 to-slate-200/60 group-hover:before:absolute group-hover:before:inset-0 truncate" href={`/apps/${appLink}`}>
            {item.name.replace(/\[.*?\]\s*/, '')}
          </Link>
        </div>
        <div className="grow">
...
//       name: 'Vercel',
//       description: 'Altan makes it easy to build extensions by providing an authentication provider that handles the OAuth flow.',
